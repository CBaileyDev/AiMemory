...
let cachedLocalBindCapability: Promise<boolean> | null = null;

/**
 * Run worker CLI command and return stdout + exit code
 * Uses async spawn so test-owned stub servers can keep serving health checks.
 */
async function runWorkerStart(envOverrides: Record<string, string> = {}): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  const child = spawn('bun', [WORKER_SCRIPT, 'start'], {
    env: {
      ...process.env,
      ...envOverrides
    },
...
async function withHealthyWorkerStub<T>(callback: (env: Record<string, string>) => Promise<T>): Promise<T> {
  const tempHome = mkdtempSync(path.join(tmpdir(), 'claude-mem-worker-json-home-'));
  const port = await new Promise<number>((resolve, reject) => {
    ...
    probe.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        probe.close();
        reject(new Error('Failed to allocate test worker port'));
        return;
      }
...
async function canBindLocalhost(): Promise<boolean> {
  if (cachedLocalBindCapability) {
    return cachedLocalBindCapability;
  }

  cachedLocalBindCapability = new Promise<boolean>(resolve => {
    const probe = net.createServer();
...
async function canRunCliIntegration(): Promise<boolean> {
  ...
  if (!(await canBindLocalhost())) {
    console.log('Skipping CLI test - localhost binding is not permitted in this environment');
    return false;
  }
...
describe('worker-json-status', () => {
  ...
  describe('start command JSON output', () => {
    describe('when worker already healthy', () => {
      it('should output valid JSON with status: ready', async () => {
        if (!(await canRunCliIntegration())) {
          return;
        }

        const { stdout, exitCode } = await withHealthyWorkerStub(runWorkerStart);
      ...
      it('should match expected JSON structure when worker is healthy', async () => {
        if (!(await canRunCliIntegration())) {
          return;
        }

        const { stdout } = await withHealthyWorkerStub(runWorkerStart);
        const parsed = JSON.parse(stdout);
  ...
  describe('Claude Code hook framework compatibility', () => {
    ...
    it('should always exit with code 0', async () => {
      ...
      const { exitCode } = await withHealthyWorkerStub(runWorkerStart);
     ...
     */
    it('should output JSON on stdout (not stderr)', async () => {
      if (!(await canRunCliIntegration())) {
        return;
      }

      const { stdout, stderr } = await withHealthyWorkerStub(runWorkerStart);
     ...
     */
    it('should always include continue: true (required for Claude Code to proceed)', async () => {
      if (!(await canRunCliIntegration())) {
        return;
      }
     ...
     */
    it('should include suppressOutput: true to hide from transcript mode', async () => {
      if (!(await canRunCliIntegration())) {
        return;
      }
     ...
     * the issue.
     */
    it('should include a valid status field', async () => {
      if (!(await canRunCliIntegration())) {
        return;
      }
