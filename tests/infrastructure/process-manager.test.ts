...
describe('ProcessManager', () => {
  ...
  beforeEach(() => {
    testDataDir = path.join(tmpdir(), `claude-mem-process-manager-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('writePidFile', () => {
    it('should create file with PID info', () => {
      ...
      const content = JSON.parse(readFileSync(pidFilePath, 'utf-8'));
  ...
  describe('touchPidFile', () => {
    it('should update mtime of existing PID file', async () => {
      ...
      const statsBefore = statSync(pidFilePath);
      ...
      const statsAfter = statSync(pidFilePath);
