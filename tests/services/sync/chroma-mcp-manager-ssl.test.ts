 ...
 *
 * Strategy: call the private buildCommandArgs() seam directly at runtime.
 * TypeScript private is compile-time only, which keeps this test fast and
 * avoids dragging the full MCP connection path into a unit-level regression.
 */
...
async function assertSslFlag(sslSetting: string | undefined, expectedValue: string) {
  ...
  const args = (mgr as any).buildCommandArgs() as string[];
  const sslIdx = args.indexOf('--ssl');
  expect(sslIdx).not.toBe(-1);
  expect(args[sslIdx + 1]).toBe(expectedValue);
}
