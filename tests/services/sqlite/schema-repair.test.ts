...
function findPythonCommand(): PythonCommand | null {
  ...
  for (const candidate of candidates) {
    const result = spawnSync(candidate.command, ['--version'], { stdio: 'ignore', timeout: 5000 });
...
function corruptDbViaPython(dbPath: string): void {
  ...
  if (!python) {
    throw new Error('Python not available for schema repair test');
  }
...
`);
  try {
    execFileSync(python.command, [...python.args, script, dbPath], { timeout: 10000 });
  } finally {
...
c.close()
`);
      execFileSync(python.command, [...python.args, scriptPath, dbPath], { timeout: 10000 });
