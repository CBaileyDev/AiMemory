...
describeWithLocalhost("Observation I/O event handlers", () => {
...
describeWithLocalhost("before_prompt_build context injection", () => {
  let workerServer: Server;
  let workerPort: number;
  let receivedRequests: Array<{ method: string; url: string; body: any }> = [];
...
describeWithLocalhost("SSE stream integration", () => {
  let server: Server;
...
describe("circuit breaker", () => {
  ...
  it("HALF_OPEN allows only a single probe — non-2xx keeps circuit open, 2xx closes it", async () => {
    ...
    try {
      ...
      const portB: number = await new Promise((resolve, reject) => {
