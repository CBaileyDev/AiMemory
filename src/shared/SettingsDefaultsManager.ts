...
export class SettingsDefaultsManager {
  ...
  private static readonly DEFAULTS: SettingsDefaults = {
    ...
    CLAUDE_MEM_TIER_SUMMARY_MODEL: 'sonnet',          // Summaries need synthesis quality — use Sonnet
    // Chroma Vector Database Configuration
    CLAUDE_MEM_CHROMA_ENABLED: 'false',        // SQLite-only by default; set to 'true' to enable Chroma vector search
    ...
    try {
      if (!existsSync(settingsPath)) {
        ...
        try {
          ...
          // stderr only: worker `start` must emit a single JSON line on stdout (hook contract)
