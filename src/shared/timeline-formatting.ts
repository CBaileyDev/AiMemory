...
/**
 * Count tokens for text using a real BPE tokenizer (cl100k_base by default).
 * Backed by `src/shared/tokenizer.ts` with a SHA-256 LRU cache and a chars/4
 * fallback if the tokenizer can't load. The name is kept as `estimateTokens`
 * for call-site compatibility; counts are accurate to within ~5% of
 * Anthropic/OpenAI ground truth.
 */
