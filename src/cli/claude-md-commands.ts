...
function estimateTokens(obs: ObservationRow): number {
  // Concatenate the fields that actually get rendered into context, then
  // tokenize once. Real BPE count via the shared tokenizer (cl100k_base),
  // cached by SHA-256 — faster than tokenizing four small strings separately.
  const combined = [obs.title, obs.subtitle, obs.narrative, obs.facts]
