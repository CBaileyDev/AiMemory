...
function getDataDir(): string {
  return process.env.CLAUDE_MEM_DATA_DIR || DEFAULT_DATA_DIR;
}
...
class Logger {
  ...
  private logDataDir: string | null = null;
  ...
  private ensureLogFileInitialized(): void {
    ...
    if (this.logFileInitialized && this.logDataDir === dataDir) return;
    this.logFileInitialized = true;
    this.logDataDir = dataDir;

    try {
      // Use default data directory to avoid circular dependency with SettingsDefaultsManager
      // but still honor CLAUDE_MEM_DATA_DIR when the process was started with one.
