...
import { resolveActiveAgentSafe } from './worker/agent-resolver.js';
...
export class WorkerService {
  ...
  /**
   * Resolve the active agent for daemon processing.
   * Falls back to Claude SDK when the selected provider is unavailable so
   * background queue processing can continue without crashing the worker.
   */
  private getActiveAgent(): SDKAgent | GeminiAgent | OpenRouterAgent {
    return resolveActiveAgentSafe({
  ...
  async processPendingQueues(sessionLimit: number = 10): Promise<{
    ...
    try {
      ...
      if (staleSessionIds.length > 0) {
        sessionStore.markSessionsFailed(staleSessionIds);
        logger.info('SYSTEM', `Marked ${staleSessionIds.length} stale sessions as failed`);

        const abandonedMessages = pendingStore.markAllSessionsMessagesAbandoned(staleSessionIds);
        if (abandonedMessages > 0) {
          logger.info('SYSTEM', `Marked ${abandonedMessages} queue messages from stale sessions as failed`);
        }
      }
    } catch (error) {
