...
export class PendingMessageStore {
  ...
  markAllSessionsMessagesAbandoned(sessionIds: number[]): number {
    ...
    const stmt = this.db.prepare(`
      UPDATE pending_messages
      SET status = 'failed', failed_at_epoch = ?
      WHERE session_db_id IN (${placeholders}) AND status IN ('pending', 'processing')
    `);

    const result = stmt.run(now, ...sessionIds);
    return result.changes;
  }
