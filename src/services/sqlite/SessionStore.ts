...
export class SessionStore {
  ...
  getStaleActiveSessionIds(thresholdEpoch: number): number[] {
    const rows = this.db.prepare(`
      SELECT id FROM sdk_sessions
      WHERE status = 'active' AND started_at_epoch < ?
    `).all(thresholdEpoch) as { id: number }[];

    return rows.map(row => row.id);
  }

  markSessionsFailed(sessionIds: number[]): void {
    ...
    `).run(nowEpoch, ...sessionIds);
  }

  getSessionStartedAtEpoch(sessionDbId: number): number | null {
