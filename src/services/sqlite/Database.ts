...
const UV_COMMON_PATHS = process.platform === 'win32'
...
function findPythonBinary(): PythonCommand {
  ...
  for (const candidate of pythonCandidates) {
    if (canExecute(candidate.command, ['--version'])) {
      return candidate;
    }
  }

  const uvCandidates = ['uv', ...UV_COMMON_PATHS.filter(path => path !== 'uv')];
  for (const uvPath of uvCandidates) {
    if (canExecute(uvPath, ['run', '--no-project', 'python', '--version'])) {
      return {
        command: uvPath,
  ...
  throw new Error(
    'Python is required to repair malformed SQLite schemas, but no usable runtime was found. '
    + 'Tried python3, python, and "uv run --no-project python". '
    + 'Install Python 3 or uv, then retry.'
  );
}
...
c.close()
`);
      execFileSync(python.command, [...python.args, scriptPath, dbPath, objectName], { timeout: 10000 });
