 ...
 * Uses the shared BPE tokenizer (cl100k_base) with a SHA-256 LRU cache so
 * repeated calls for the same observation are effectively free. Falls back
 * to chars/4 if the tokenizer can't load.
 */
export function calculateObservationTokens(obs: Observation): number {
  // Concatenate rendered fields once; one tokenization is cheaper than four.
  const combined = [
    obs.title ?? '',
    obs.subtitle ?? '',
    obs.narrative ?? '',
