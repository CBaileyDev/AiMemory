...
function getDataDirectory(): string {
  return process.env.CLAUDE_MEM_DATA_DIR || path.join(homedir(), '.claude-mem');
}
...
export function readPidFile(): PidInfo | null {
  ...
  try {
    return JSON.parse(readFileSync(pidFilePath, 'utf-8'));
  } catch (error) {
    logger.warn('SYSTEM', 'Failed to parse PID file', { path: pidFilePath }, error as Error);
    return null;
  }
}
...
export function removePidFile(): void {
  ...
  try {
    ...
    logger.warn('SYSTEM', 'Failed to remove PID file', { path: pidFilePath }, error as Error);
  }
}
...
export function runOneTimeChromaMigration(dataDirectory?: string): void {
  const effectiveDataDir = dataDirectory ?? getDataDirectory();
...
export function touchPidFile(): void {
  ...
  try {
    if (!existsSync(pidFilePath)) return;
    const now = new Date();
    utimesSync(pidFilePath, now, now);
  } catch {
