...
import { isProviderConfigurationError, resolveActiveAgent } from '../../agent-resolver.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' {
    ...
    if (agent === this.openRouterAgent) {
      return 'openrouter';
    }
    return agent === this.geminiAgent ? 'gemini' : 'claude';
  }
  ...
  private ensureGeneratorRunning(sessionDbId: number, source: string): void {
    ...
    const sessionOriginMs = this.dbManager.getSessionStore().getSessionStartedAtEpoch(sessionDbId) ?? session.startTime;
              ...
              setTimeout(() => {
                ...
                if (stillExists && !stillExists.generatorPromise) {
                  ...
                  try {
                    ...
                    logger.error('SESSION', 'Crash recovery blocked by provider configuration', {
                      sessionId: sessionDbId
                    }, error as Error);
                  }
                }
