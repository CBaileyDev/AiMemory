...
import { countTokens } from '../../../shared/tokenizer.js';
...
export class CorpusRenderer {
  ...
  renderCorpus(corpus: CorpusFile): string {
    ...
    logger.debug('WORKER', 'Rendered knowledge corpus', {
      name: corpus.name,
  ...
  /**
   * Count tokens for rendered corpus text.
   * Uses the shared BPE tokenizer (cl100k_base) with a SHA-256 LRU cache,
   * falling back to chars/4 if the tokenizer can't load.
   */
  ...
  generateSystemPrompt(corpus: CorpusFile): string {
    ...
    logger.debug('WORKER', 'Generated corpus system prompt', {
      name: corpus.name,
      observationCount: corpus.stats.observation_count
    });
    return prompt;
  }
}
