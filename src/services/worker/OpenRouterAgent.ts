...
export class OpenRouterAgent extends BaseRESTAgent {
  ...
  protected validateConfig(): void {
    const { apiKey } = this.getOpenRouterConfig();
    if (!apiKey) {
  ...
  protected async queryModel(history: ConversationMessage[]): Promise<ModelQueryResult> {
    ...
    const response = await this.queryOpenRouterMultiTurn(history, apiKey, model, siteUrl, appName);
    return { ...response, modelId: model };
  }

  /**
   * Count tokens using the shared BPE tokenizer (cl100k_base).
   * Falls back to chars/4 if the tokenizer can't load.
   */
